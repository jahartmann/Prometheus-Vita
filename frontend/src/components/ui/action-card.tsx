...
export function ActionCard({ tone, icon: Icon, title, description, badge, href, actionLabel }: ActionCardProps) {
  return (
    <Link href={href} className="block h-full">
      <Card hover className={cn("h-full overflow-hidden", accentClass[tone])}>
        <CardContent className="flex h-full flex-col gap-4 p-4">
          ...
          <span className="mt-auto inline-flex w-fit items-center rounded-md border bg-background px-3 py-1.5 text-sm font-medium">
            {actionLabel}
