...
func (t *VMFileWriteTool) Parameters() json.RawMessage {
	return json.RawMessage(`{
		...
		"properties": {
			...
			"dry_run": {
				"type": "boolean",
				"description": "Wenn true, wird nur eine Schreibvorschau erzeugt. Standard: true fuer Sicherheitsvorschau"
			}
		},
...
func (t *VMFileWriteTool) Preview(ctx context.Context, args json.RawMessage) (json.RawMessage, error) {
	var params struct {
		NodeID  string `json:"node_id"`
		VMID    int    `json:"vmid"`
		VMType  string `json:"vm_type"`
	...
	return json.Marshal(map[string]interface{}{
		"dry_run":       true,
		"action":        "vm_file_write",
		"node_id":       params.NodeID,
		"vmid":          params.VMID,
		"vm_type":       params.VMType,
		"path":          params.Path,
		"content_bytes": len(params.Content),
		"message":       "Dateipfad und Inhalt validiert, aber nicht geschrieben. Fuer echtes Schreiben dry_run=false setzen und Approval freigeben.",
	})
}
